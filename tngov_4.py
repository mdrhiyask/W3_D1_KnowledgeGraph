...
# Quick CLI Testing Execution
if __name__ == "__main__":
    load_dotenv()

    api_key = os.getenv("OPENAI_API_KEY")
    if not api_key:
        print("Please export OPENAI_API_KEY before testing.")
    else:
        print("Initializing GraphRAG Query Engine...")
        engine = GraphRAGEngine(api_key=api_key)
        
        test_query = "What schemes offer drip irrigation subsidies for small farmers?"
        print(f"\n[Test Query]: {test_query}")
        result = engine.query(test_query)
        
        print("\n--- Final GraphRAG Response ---")
        print(result["answer"])